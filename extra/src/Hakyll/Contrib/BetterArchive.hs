...
buildArchivePaginateWith ::
  ...
  (ArchiveDateData -> m [[Identifier]]) ->
  -- | Select items to paginate
  Pattern ->
  -- | Identifiers for the pages
  (ArchiveDate -> Identifier) ->
...
archivePageStrategy dates =
  let grouped = M.foldrWithKey groupByYear M.empty dates
   in return $ map snd $ M.toDescList grouped
  where
    groupByYear id' date groups =
      let year = archiveExtractYear date
       in M.insertWith (++) year [id'] groups
