...
[1] As much as I would also like to provide instructions for native Windows, 
being a Windows power-user myself, the preferred method for emulating a Unix 
...
[2] The astute reader might have noticed that we are not using the latest 
version of MLton. That is because the Github release for it with glibc 2.23 is 
broken (bad ld.so in the mlton-compile executable, "only" requires a minor 
change to the released bin/mlton script that is more trouble than it's worth 
to put here), and the other version is for glibc 2.31 which is too new for 
Debian.
