...
This lab is all about *instrumenting code* and then *proving properties about
the original code through the instrumented version*. You may be wondering what
...
Anyways back to safety: a ***safety property*** is class[^2] of traces that can be
...
There is a formula called a ***box modality***, which we denote as $$[\alpha]P$$
...
can we reconcile this? Simple: we *rewrite the program so that any operation
...
[^4]: Consider: a program that does something unsafe only after (safely)
