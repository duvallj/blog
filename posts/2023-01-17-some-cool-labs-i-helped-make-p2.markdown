...
This lab is all about *writing an automated theorem prover* for a limited set
...
The answer is, *it depends on your interpretation of $$A$$*. And I mean that
...
are really asking whether it is *true in all interpretations*. Here's some
symbols:
...
<div class="center">
<p>`door` says `canopen(jack)`</p>
<p>`door` says forall `p`. (`jack` says `canopen(p)`) implies `canopen(p)`</p>
</div>
...
Certificate Authorities, and it all gets pretty complicated, but! *There is a
reason for all of it* and *it can be automated very naturally*.
...
[^3]: As an exercise, convince yourself that all the proof rules I've shown are
    true!
[^4]: Working with these "says" clauses is a little tricky and has a lot of
